...
export default function ViewEvent() {
  ...
  const fetchEvent = async () => {
    ...
    if (error) {
      ...
      setEvent(data as unknown as FetchedEvent);
    }
    setLoading(false);
  };
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['bottom', 'left', 'right']}>
      <Stack.Screen 
        options={{
          title: 'Event Details',
          headerRight: () => (
            <TouchableOpacity onPress={() => router.push(`/events/edit?id=${id}`)}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={[styles.card, { backgroundColor: colors.backgroundElement || (scheme === 'dark' ? '#1c1c1e' : '#ffffff') }]}>
          ...
          {event.location && (
            <View style={styles.detailRow}>
              ...
              <View>
                ...
                <Text style={[styles.value, { color: colors.text }]}>{event.location}</Text>
              </View>
            </View>
          )}
          ...
          {event.recurrence && (
            <View style={styles.detailRow}>
              <Ionicons name="repeat" size={20} color={colors.primary} style={styles.icon} />
              <View>
                <Text style={[styles.label, { color: colors.text + '80' }]}>Recurrence</Text>
                <Text style={[styles.value, { color: colors.text }]}>{event.recurrence}</Text>
              </View>
            </View>
          )}
        ...
        >
          ...
          <Text style={styles.deleteButtonText}>{deleting ? 'Cancelling...' : 'Cancel Event'}</Text>
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
